...
function computeStats(pitches,abs,runsByInning={},earnedRunsByInning={},baseEvts=[]){
  ...
  const lSplit=mkSplit(abs.filter(ab=>ab.hand==='L'),pitches.filter(p=>p.hand==='L')),rSplit=mkSplit(abs.filter(ab=>ab.hand==='R'),pitches.filter(p=>p.hand==='R'));
...
// ── SMOOTH DENSITY ZONE — Gaussian blur applied to density layer ──
function DensityZone({pitches=[],filterType='all',hitsOnly=false}){
  ...
  return(
    <svg viewBox={`0 0 ${W} ${H}`} style={{width:'100%',maxWidth:220,height:'auto',display:'block',margin:'0 auto'}}>
      ...
      {/* Blurred smooth density layer */}
      <g filter="url(#dblur)">
        {grid.map((row,ri)=>row.map((v,ci)=>{const color=getColor(v);return color?<rect key={`${ri}-${ci}`} x={ci*cW-1} y={ri*cH-1} width={cW+2} height={cH+2} fill={color}/>:null;}))}
      </g>
      {/* Zone overlay on top — no blur */}
...
function HardHitZoneMap({pitches=[],mode='hardHit',onModeChange}){
  const zoneData=useMemo(()=>Array.from({length:9},(_,i)=>{
    const zone=i+1;
    const inPlay=pitches.filter(p=>p.zone===zone&&p.result==='InPlay');
    ...
    const hardRate=inPlay.length>0?hardHit.length/inPlay.length:null;
    const ba=inPlay.length>0?hits.length/inPlay.length:null;
    return{zone,total:inPlay.length,hardHit:hardHit.length,hits:hits.length,hardRate,ba};
  }),[pitches]);
  ...
  const maxVal=Math.max(...zoneData.map(z=>mode==='hardHit'?(z.hardRate||0):(z.ba||0)),0.01);

  return(
    <div>
      <div style={{display:'flex',gap:'4px',marginBottom:'10px',justifyContent:'center'}}>
        {[{k:'hardHit',l:'Hard Hit %'},{k:'ba',l:'BA by Zone'}].map(({k,l})=>(
          <button key={k} onClick={()=>onModeChange(k)} style={{padding:'4px 12px',borderRadius:'5px',border:'1.5px solid #475569',cursor:'pointer',fontSize:'10px',fontWeight:'800',background:mode===k?'#475569':'transparent',color:mode===k?'#fff':'#475569',fontFamily:'inherit'}}>
            {l}
          </button>
        ))}
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{width:'100%',maxWidth:260,height:'auto',display:'block',margin:'0 auto'}}>
        ...
        <rect width={W} height={H} fill="#060d1a" rx="8"/>
        <rect width={W} height={H} fill="url(#hzGrad)" rx="8"/>
        {/* Colored zones */}
        {zoneData.map(({zone,hardRate,ba,total,hardHit,hits})=>{
          const c=(zone-1)%3,r=Math.floor((zone-1)/3);
          const rate=mode==='hardHit'?hardRate:ba;
          const color=getZoneColor(rate);
          const displayVal=rate!==null?Math.round(rate*100)+'%':'—';
          const subVal=mode==='hardHit'?`${hardHit}/${total} BIP`:`${hits}/${total} BIP`;
          return(
            <g key={zone}>
              <rect x={ZL+c*CW+1} y={ZT+r*CR+1} width={CW-2} height={CR-2} fill={color} rx="3"/>
              {total>0?(
                <>
                  <text x={ZL+c*CW+CW/2} y={ZT+r*CR+CR/2-3} textAnchor="middle" fill="#fff" fontSize="16" fontWeight="900" fontFamily="monospace" style={{textShadow:'0 1px 4px rgba(0,0,0,0.8)'}}>{displayVal}</text>
                  <text x={ZL+c*CW+CW/2} y={ZT+r*CR+CR/2+12} textAnchor="middle" fill="rgba(255,255,255,0.55)" fontSize="8" fontFamily="monospace">{subVal}</text>
                </>
              ):(
        ...
        <g transform={`translate(6,${H-14})`}>
          {['#22c55e','#f59e0b','#ef4444'].map((c,i)=><rect key={i} x={i*12} y={0} width={11} height={7} fill={c} rx="1" opacity="0.8"/>)}
          <text x="0" y="16" fill="#334155" fontSize="7.5" fontFamily="monospace">low → high {mode==='hardHit'?'hard contact':'batting avg'}</text>
        </g>
      </svg>
      <div style={{marginTop:'10px',fontSize:'9px',color:'#334155',textAlign:'center',lineHeight:'1.5'}}>
        {mode==='hardHit'?'Hard hit = Line drives + Hard grounders/fly balls':'BA = Hits ÷ balls in play per zone'}
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  const thSort=(col,label)=>{
    ...
    return(
      <th onClick={()=>setTeamSort(prev=>prev.col===col?{col,dir:prev.dir==='asc'?'desc':'asc'}:{col,dir:'asc'})}
        style={{textAlign:'left',padding:'8px 10px',borderBottom:'1px solid #1e3a5f',color:active?'#60a5fa':'#334155',fontWeight:'800',fontSize:'9px',letterSpacing:'0.5px',cursor:'pointer',whiteSpace:'nowrap',userSelect:'none'}}>
        {label}{active?(teamSort.dir==='asc'?' ↑':' ↓'):''}
      </th>
    );
  };
    ...
    <div style={{fontFamily:'"DM Mono","IBM Plex Mono","Fira Code",monospace',background:'#030712',minHeight:'100vh',color:'#e2e8f0'}}>
      <style>{`
        ...
        *{box-sizing:border-box;}select option{background:#060d1a;}
        ...
        .be-btn:hover{opacity:0.8;}.hist-row:hover td{background:#0a1929!important;}
        .team-row:hover td{background:#0a1929!important;cursor:pointer;}
        th.sortable:hover{color:#60a5fa!important;}
      `}</style>
              ...
              {pitchers.map((name,i)=>(
                <div key={name} className="mgmt-row" style={{display:'flex',alignItems:'center',gap:'8px',padding:'10px 8px',borderRadius:'8px',borderBottom:'1px solid #1e3a5f',transition:'background 0.1s'}}>
                  <div style={{display:'flex',flexDirection:'column',gap:'2px'}}>
                    <button onClick={()=>movePitcher(i,-1)} disabled={i===0} style={{width:'22px',height:'18px',borderRadius:'4px',border:'1px solid #1e3a5f',background:'transparent',color:i===0?'#1e3a5f':'#60a5fa',cursor:i===0?'default':'pointer',fontSize:'10px',display:'flex',alignItems:'center',justifyContent:'center'}}>▲</button>
                    <button onClick={()=>movePitcher(i,1)} disabled={i===pitchers.length-1} style={{width:'22px',height:'18px',borderRadius:'4px',border:'1px solid #1e3a5f',background:'transparent',color:i===pitchers.length-1?'#1e3a5f':'#60a5fa',cursor:i===pitchers.length-1?'default':'pointer',fontSize:'10px',display:'flex',alignItems:'center',justifyContent:'center'}}>▼</button>
                  </div>
                  ...
                  <div style={{borderTop:'1px solid #1e3a5f',marginTop:'8px',paddingTop:'8px'}}>
                    ...
                    {[...baseEvents].reverse().slice(0,5).map((e,i)=>(
                      <div key={i} style={{display:'flex',gap:'6px',fontSize:'9px',padding:'2px 0',borderBottom:'1px solid #0a1929'}}>
                        ...
                        <span style={{fontWeight:'800',color:e.type==='SB'?'#f59e0b':e.type==='CS'||e.type==='PO_OUT'?'#22c55e':'#475569'}}>{e.type==='SB'?`SB→${e.base}`:e.type==='CS'?`CS@${e.base}`:e.type==='PO_ATT'?`PO safe ${e.base}`:`PO OUT ${e.base}`}</span>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div style={{padding:'14px 16px',maxWidth:'1320px',margin:'0 auto'}}>
          <div style={{...card({padding:'12px 16px',marginBottom:'10px',display:'flex',flexWrap:'wrap',gap:'10px',alignItems:'flex-end'})}}>
            ...
            {ana.pitcher&&(
              <div style={{display:'flex',gap:'6px',alignItems:'flex-end'}}>
                ...
                {ana.outingId!=='all'&&(<>
                  <button onClick={generatePDFReport} style={{padding:'7px 12px',borderRadius:'6px',border:'1px solid #2d6a9f',background:'rgba(45,106,159,0.1)',color:'#60a5fa',cursor:'pointer',fontSize:'11px',fontWeight:'700',fontFamily:'inherit',whiteSpace:'nowrap',marginBottom:'1px'}}>📄 PDF</button>
                  <button className="del-btn" onClick={()=>deleteOuting(ana.outingId)} style={{padding:'7px 12px',borderRadius:'6px',border:'1px solid rgba(239,68,68,0.3)',background:'transparent',color:'#ef4444',cursor:'pointer',fontSize:'11px',fontWeight:'700',fontFamily:'inherit',whiteSpace:'nowrap',marginBottom:'1px'}}>🗑 Delete</button>
                </>)}
              </div>
            )}
          ...
          {!ana.pitcher||!anaData.pitches.length?(
            <div style={{...card(),textAlign:'center',padding:'60px',color:'#1e3a5f'}}>
              ...
              <div style={{fontSize:'12px',marginTop:'6px',color:'#334155'}}>Chart live outings first, then analyze here</div>
            </div>
          ):(
            <>
              {show('stats')&&s&&(
              ...
              {show('hardzone')&&(
                <div style={{...card({marginBottom:'14px'})}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:vw==='hardzone'?'280px 1fr':'280px 1fr',gap:'24px',alignItems:'start'}}>
                    ...
                    <div>
                      <div style={{fontSize:'11px',fontWeight:'700',color:'#475569',marginBottom:'14px'}}>Zone-by-zone breakdown</div>
                      <table style={{width:'100%',borderCollapse:'collapse',fontSize:'11px',fontFamily:'monospace',maxWidth:'500px'}}>
                        <thead><tr>{['Zone','BIP','Hard Hit','Hard Hit%','Hits','BA'].map(h=><th key={h} style={{textAlign:'left',padding:'5px 10px',borderBottom:'1px solid #1e3a5f',color:'#334155',fontWeight:'800',fontSize:'9px',letterSpacing:'0.5px'}}>{h}</th>)}</tr></thead>
                        <tbody>
                          {Array.from({length:9},(_,i)=>{
                            const zone=i+1;
                            const inPlay=(anaData.filtPitches||anaData.pitches).filter(p=>p.zone===zone&&p.result==='InPlay');
                            const hardHit=inPlay.filter(p=>p.hitType==='LD'||(p.hitStr==='Hard'&&p.hitType!=='PU'));
                            const hits=inPlay.filter(p=>['1B','2B','3B','HR'].includes(p.hitResult));
                            ...
                            return(
                              <tr key={zone} style={{borderBottom:'1px solid #0a1929'}} onMouseEnter={e=>e.currentTarget.style.background='#0a1929'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                                ...
                                <td style={{padding:'6px 10px',color:'#94a3b8'}}>{hardHit.length}</td>
                                <td style={{padding:'6px 10px',fontWeight:'800',color:hr>=40?'#ef4444':hr>=25?'#f59e0b':hr!==null?'#22c55e':'#334155'}}>{hr!==null?hr+'%':'—'}</td>
                                <td style={{padding:'6px 10px',color:'#94a3b8'}}>{hits.length}</td>
                                <td style={{padding:'6px 10px',fontWeight:'800',color:ba>=300?'#ef4444':ba>=250?'#f59e0b':ba!==null?'#22c55e':'#334155'}}>{ba!==null?'.'+String(ba).padStart(3,'0'):'—'}</td>
                              </tr>
                            );
                          })}
                        </tbody>
                      </table>
                      <div style={{marginTop:'12px',fontSize:'9px',color:'#334155',lineHeight:'1.6'}}>
                        <div><span style={{color:'#ef4444',fontWeight:'700'}}>Red</span> = High danger zone (≥40% hard hit or ≥.300 BA)</div>
                        <div><span style={{color:'#f59e0b',fontWeight:'700'}}>Amber</span> = Moderate (25–39% or .250–.299)</div>
                        <div><span style={{color:'#22c55e',fontWeight:'700'}}>Green</span> = Well controlled (&lt;25% or &lt;.250)</div>
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    {anaData.stats&&COUNTS.map(c=>{
                      ...
                      return(
                        <div key={c} style={{padding:'10px',background:'#060d1a',borderRadius:'8px',border:'1px solid #0d1f33',opacity:mix.total===0?0.35:1}}>
                          <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'8px'}}><span style={{fontSize:'15px',fontWeight:'900',color:'#e2e8f0',fontFamily:'monospace',minWidth:'34px'}}>{c}</span><span style={{fontSize:'10px',color:'#475569',fontWeight:'700'}}>{mix.total}p</span><div style={{marginLeft:'auto',display:'flex',gap:'5px'}}>{outPct!=null&&<span style={{fontSize:'10px',fontWeight:'800',color:outPct>=60?'#22c55e':outPct>=40?'#f59e0b':'#ef4444',padding:'1px 5px',borderRadius:'3px',background:outPct>=60?'rgba(34,197,94,0.08)':outPct>=40?'rgba(245,158,11,0.08)':'rgba(239,68,68,0.08)'}}>{outPct}% OUT</span>}{hitPct!=null&&hitPct>0&&<span style={{fontSize:'10px',fontWeight:'800',color:'#f43f5e',background:'rgba(244,63,94,0.08)',padding:'1px 5px',borderRadius:'3px'}}>{hitPct}% HIT</span>}</div></div>
                          {mix.total>0?(<div style={{display:'flex',flexDirection:'column',gap:'5px'}}>
                            <div><div style={{fontSize:'8px',fontWeight:'700',color:'#334155',textTransform:'uppercase',letterSpacing:'0.8px',marginBottom:'2px'}}>Pitch Mix</div><div style={{display:'flex',height:'7px',borderRadius:'3px',overflow:'hidden',gap:'1px'}}>{usedTypes.map(t=>(<div key={t} style={{width:mix.byType[t].pct+'%',background:PC[t],minWidth:'3px'}} title={`${t}: ${mix.byType[t].pct}%`}/>))}</div><div style={{display:'flex',flexWrap:'wrap',gap:'3px',marginTop:'3px'}}>{usedTypes.map(t=>(<div key={t} style={{display:'flex',alignItems:'center',gap:'2px',fontSize:'9px'}}><div style={{width:'5px',height:'5px',borderRadius:'2px',background:PC[t]}}/><span style={{color:'#e2e8f0',fontFamily:'monospace',fontWeight:'700'}}>{t} {mix.byType[t].pct}%</span></div>))}</div></div>
                            <div><div style={{display:'flex',height:'6px',borderRadius:'3px',overflow:'hidden',gap:'1px'}}>{mix.strikePct>0&&<div style={{width:mix.strikePct+'%',background:'#22c55e',minWidth:'3px'}}/>}{mix.ballPct>0&&<div style={{width:mix.ballPct+'%',background:'#ef4444',minWidth:'3px'}}/>}{mix.inPlayPct>0&&<div style={{width:mix.inPlayPct+'%',background:'#f59e0b',minWidth:'3px'}}/>}</div><div style={{display:'flex',gap:'8px',marginTop:'2px',fontSize:'9px'}}>{mix.strikePct>0&&<span style={{color:'#22c55e',fontWeight:'700'}}>K {mix.strikePct}%</span>}{mix.ballPct>0&&<span style={{color:'#ef4444',fontWeight:'700'}}>B {mix.ballPct}%</span>}{mix.inPlayPct>0&&<span style={{color:'#f59e0b',fontWeight:'700'}}>IP {mix.inPlayPct}%</span>}</div></div>
              ...
              {show('arsenal')&&(
                <div style={{...card({marginBottom:'14px'})}}>
                  ...
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:'12px',fontFamily:'monospace'}}>
                    ...
                    <tbody>{anaData.stats&&PT.filter(t=>anaData.stats.ptBreak[t]?.n>0).sort((a,b)=>anaData.stats.ptBreak[b].n-anaData.stats.ptBreak[a].n).map(t=>{const d=anaData.stats.ptBreak[t];return(<tr key={t} style={{borderBottom:'1px solid #0a1929'}}><td style={{padding:'10px 12px'}}><span style={{background:PC[t],color:'#fff',padding:'3px 8px',borderRadius:'4px',fontWeight:'800',fontSize:'11px'}}>{t}</span><span style={{marginLeft:'8px',color:'#475569',fontSize:'11px'}}>{PL[t]}</span></td><td style={{padding:'10px 12px',color:'#64748b',fontWeight:'700'}}>{d.n}</td><td style={{padding:'10px 12px'}}><div style={{display:'flex',alignItems:'center',gap:'8px'}}><div style={{width:'80px',height:'6px',background:'#0a1929',borderRadius:'3px',overflow:'hidden'}}><div style={{width:d.pct+'%',height:'100%',background:PC[t],borderRadius:'3px'}}/></div><span style={{color:'#e2e8f0',fontWeight:'700'}}>{d.pct}%</span></div></td><td style={{padding:'10px 12px',color:d.n>0&&Math.round(d.whiff/d.n*100)>=25?'#22c55e':'#64748b',fontWeight:'800'}}>{d.n>0?Math.round(d.whiff/d.n*100)+'%':'—'}</td><td style={{padding:'10px 12px',color:'#e2e8f0',fontWeight:'900',fontSize:'13px'}}>{d.avgVel||'—'}</td><td style={{padding:'10px 12px',color:d.n>0&&Math.round(d.ball/d.n*100)>=40?'#f43f5e':'#64748b',fontWeight:'700'}}>{d.n>0?Math.round(d.ball/d.n*100)+'%':'—'}</td></tr>);})}</tbody>
                  </table>
                </div>
              )}
              ...
              {show('history')&&(
                <div style={{...card({marginBottom:'14px'})}}>
                  ...
                  {anaData.outings.length===0?(
                    <div style={{color:'#334155',fontSize:'12px',fontStyle:'italic'}}>No outings recorded yet</div>
                  ):(
                    <>
                      <div style={{overflowX:'auto'}}>
                        <table style={{width:'100%',borderCollapse:'collapse',fontSize:'11px',fontFamily:'monospace'}}>
                          ...
                          <tbody>
                            {[...anaData.outings].sort((a,b)=>b.date.localeCompare(a.date)).map(outing=>{
                              ...
                              if(!os)return null;
                              const eraColor=parseFloat(os.era)<=2?'#22c55e':parseFloat(os.era)<=4?'#f59e0b':'#ef4444';
                              return(
                                <tr key={outing.id} className="hist-row" style={{borderBottom:'1px solid #0a1929',transition:'background 0.1s'}}>
                                  ...
                                  <td style={{padding:'7px 10px',color:'#94a3b8'}}>{outing.opponent||'—'}</td>
                                  <td style={{padding:'7px 10px',color:'#60a5fa',fontWeight:'800'}}>{os.ip}</td>
                                  <td style={{padding:'7px 10px',color:'#475569'}}>{os.tp}</td>
                                  <td style={{padding:'7px 10px',color:'#ef4444',fontWeight:'700'}}>{os.ks}</td>
                                  <td style={{padding:'7px 10px',color:'#f59e0b',fontWeight:'700'}}>{os.walks}</td>
                                  <td style={{padding:'7px 10px',color:'#94a3b8'}}>{os.hits}</td>
                                  <td style={{padding:'7px 10px',color:os.totalRuns>0?'#f43f5e':'#475569',fontWeight:os.totalRuns>0?'700':'400'}}>{os.totalRuns}</td>
                                  <td style={{padding:'7px 10px',color:os.totalEarnedRuns>0?'#f97316':'#475569',fontWeight:os.totalEarnedRuns>0?'700':'400'}}>{os.totalEarnedRuns}</td>
                                  <td style={{padding:'7px 10px',fontWeight:'900',color:eraColor}}>{os.era}</td>
                                  <td style={{padding:'7px 10px',color:'#94a3b8'}}>{os.kbb}</td>
                                  <td style={{padding:'7px 10px',color:'#94a3b8'}}>{os.whiffPct}</td>
                                  <td style={{padding:'7px 10px',color:os.sbAllowed>0?'#f59e0b':'#475569',fontWeight:os.sbAllowed>0?'700':'400'}}>{os.sbAllowed}</td>
                                  <td style={{padding:'7px 10px',color:os.poOut>0?'#22c55e':'#475569',fontWeight:os.poOut>0?'700':'400'}}>{os.poOut}</td>
                                </tr>
                              );
                            })}
                          </tbody>
                          ...
                          {s&&(
                            <tfoot>
                              <tr style={{borderTop:'2px solid #1e3a5f',background:'rgba(30,58,95,0.15)'}}>
                                <td style={{padding:'8px 10px',fontWeight:'900',color:'#60a5fa',fontSize:'10px'}} colSpan={2}>SEASON TOTALS</td>
                                <td style={{padding:'8px 10px',color:'#60a5fa',fontWeight:'900'}}>{s.ip}</td>
                                <td style={{padding:'8px 10px',color:'#475569',fontWeight:'700'}}>{s.tp}</td>
                                <td style={{padding:'8px 10px',color:'#ef4444',fontWeight:'900'}}>{s.ks}</td>
                                <td style={{padding:'8px 10px',color:'#f59e0b',fontWeight:'900'}}>{s.walks}</td>
                                <td style={{padding:'8px 10px',color:'#94a3b8',fontWeight:'700'}}>{s.hits}</td>
                                <td style={{padding:'8px 10px',color:'#f43f5e',fontWeight:'900'}}>{s.totalRuns}</td>
                                <td style={{padding:'8px 10px',color:'#f97316',fontWeight:'900'}}>{s.totalEarnedRuns}</td>
                                <td style={{padding:'8px 10px',fontWeight:'900',color:parseFloat(s.era)<=2?'#22c55e':parseFloat(s.era)<=4?'#f59e0b':'#ef4444'}}>{s.era}</td>
                                <td style={{padding:'8px 10px',color:'#94a3b8',fontWeight:'700'}}>{s.kbb}</td>
                                <td style={{padding:'8px 10px',color:'#94a3b8',fontWeight:'700'}}>{s.whiffPct}</td>
                                <td style={{padding:'8px 10px',color:s.sbAllowed>0?'#f59e0b':'#475569',fontWeight:'700'}}>{s.sbAllowed}</td>
                                <td style={{padding:'8px 10px',color:s.poOut>0?'#22c55e':'#475569',fontWeight:'700'}}>{s.poOut}</td>
                              </tr>
                            </tfoot>
                          )}
                        </table>
      ...
      {tab==='team'&&(
        <div style={{padding:'14px 16px',maxWidth:'1320px',margin:'0 auto'}}>
          <div style={{...card({padding:'16px',marginBottom:'14px'})}}>
            <div style={{fontSize:'18px',fontWeight:'900',color:'#e2e8f0',fontFamily:'"Space Grotesk",sans-serif',marginBottom:'4px'}}>👥 Team Overview</div>
          ...
          {!teamData.teamStats?(
            <div style={{...card(),textAlign:'center',padding:'60px',color:'#1e3a5f'}}>
              ...
              <div style={{fontSize:'12px',marginTop:'6px',color:'#334155'}}>Chart outings in the Charting tab first</div>
            </div>
          ):(
            <>
              {/* Team combined stats */}
              <div style={{...card({marginBottom:'14px'})}}>
                ...
                {(()=>{const ts=teamData.teamStats;return(
                  <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                    {[[{v:ts.ip,l:'IP'},{v:ts.bf,l:'BF'},{v:ts.tp,l:'Pitches'},{v:ts.pip,l:'P/IP'},{v:ts.pbf,l:'P/BF'},{v:outings.length,l:'Outings'}],[{v:ts.era,l:'ERA',ac:'#f43f5e'},{v:ts.kbb,l:'K/BB'},{v:ts.lobPct,l:'LOB%'},{v:ts.totalRuns,l:'Runs',ac:'#f43f5e'},{v:ts.hits,l:'Hits'},{v:ts.walks,l:'BB'}],[{v:ts.ks,l:"K's",ac:'#ef4444'},{v:ts.whiffPct,l:'Whiff%'},{v:ts.babip,l:'BABIP'},{v:ts.baRisp,l:'BA/RISP'},{v:ts.sbAllowed,l:'SB Allow',ac:'#f59e0b'},{v:ts.csOut,l:'CS',ac:'#22c55e'}]].map((row,ri)=>(<div key={ri} style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:'8px'}}>{row.map(({v,l,ac},ci)=><StatBox key={ci} v={v} l={l} accent={ac}/>)}</div>))}
                  </div>
                )})()}
              </div>
              ...
              {teamData.pitcherData.length>0&&(
                <div style={{...card({marginBottom:'14px'})}}>
                  <div style={{fontSize:'9px',fontWeight:'800',color:'#2d6a9f',textTransform:'uppercase',letterSpacing:'2px',marginBottom:'8px',fontFamily:'"Space Grotesk",sans-serif',display:'flex',alignItems:'center',gap:'8px'}}>
                    Pitcher Roster
                    <span style={{fontSize:'9px',color:'#334155',fontWeight:'600',textTransform:'none',letterSpacing:'0'}}>— click column headers to sort</span>
                  </div>
                  <div style={{overflowX:'auto'}}>
                    <table style={{width:'100%',borderCollapse:'collapse',fontSize:'11px',fontFamily:'monospace'}}>
                      <thead>
                      ...
                      <tbody>
                        {sortedPitcherData.map(({name,stats:ps,outings:outCnt})=>{
                          const eraC=parseFloat(ps.era)<=2?'#22c55e':parseFloat(ps.era)<=4?'#f59e0b':'#ef4444';
                          return(
                            <tr key={name} className="team-row" style={{borderBottom:'1px solid #0a1929',transition:'background 0.1s'}} onClick={()=>{setAna(p=>({...p,pitcher:name,outingId:'all',view:'all'}));setTab('analytics');}}>
                              ...
                              <td style={{padding:'8px 10px',color:'#60a5fa',fontWeight:'800'}}>{ps.ip}</td>
                              <td style={{padding:'8px 10px',fontWeight:'900',color:eraC}}>{ps.era}</td>
                              <td style={{padding:'8px 10px',color:'#94a3b8'}}>{ps.kbb}</td>
                              <td style={{padding:'8px 10px',color:'#ef4444',fontWeight:'700'}}>{ps.ks}</td>
                              <td style={{padding:'8px 10px',color:'#f59e0b',fontWeight:'700'}}>{ps.walks}</td>
                              <td style={{padding:'8px 10px',color:'#94a3b8'}}>{ps.hits}</td>
                              <td style={{padding:'8px 10px',color:ps.totalRuns>0?'#f43f5e':'#475569',fontWeight:ps.totalRuns>0?'700':'400'}}>{ps.totalRuns}</td>
                              <td style={{padding:'8px 10px',color:'#94a3b8'}}>{ps.whiffPct}</td>
                              <td style={{padding:'8px 10px',color:'#94a3b8'}}>{ps.babip}</td>
                              <td style={{padding:'8px 10px',color:ps.sbAllowed>0?'#f59e0b':'#475569',fontWeight:ps.sbAllowed>0?'700':'400'}}>{ps.sbAllowed}</td>
                              <td style={{padding:'8px 10px',color:ps.poOut>0?'#22c55e':'#475569',fontWeight:ps.poOut>0?'700':'400'}}>{ps.poOut}</td>
                  ...
                  <div style={{marginTop:'10px',fontSize:'9px',color:'#334155'}}>Click any pitcher row to jump to their Analytics</div>
                </div>
              )}
                        ...
                        <div style={{display:'flex',flexDirection:'column',gap:'4px'}}>
                          {PT.filter(t=>ps.ptBreak[t]?.n>0).sort((a,b)=>ps.ptBreak[b].pct-ps.ptBreak[a].pct).map(t=>{const d=ps.ptBreak[t];return(
                            <div key={t} style={{display:'flex',alignItems:'center',gap:'5px'}}>
                              ...
                              <span style={{color:'#94a3b8',fontWeight:'700',fontSize:'9px',minWidth:'24px',fontFamily:'monospace'}}>{d.pct}%</span>
                              {d.avgVel&&<span style={{color:'#475569',fontSize:'8px',fontFamily:'monospace'}}>{d.avgVel}</span>}
                            </div>
                          );})}
                        </div>
                        <div style={{display:'flex',gap:'10px',marginTop:'8px',fontSize:'9px',color:'#475569'}}>
                          <span>ERA: <span style={{color:parseFloat(ps.era)<=3?'#22c55e':'#f59e0b',fontWeight:'700'}}>{ps.era}</span></span>
                          <span>K/BB: <span style={{color:'#e2e8f0',fontWeight:'700'}}>{ps.kbb}</span></span>
                        </div>
                      </div>
                    ))}
                  </div>
